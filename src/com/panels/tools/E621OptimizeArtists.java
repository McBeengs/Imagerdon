...
public class E621OptimizeArtists {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    ResultSet rs = conn.createStatement().executeQuery("SELECT name FROM artist WHERE server = 3");
                    while (rs.next()) {
                    ...
                    for (String s : onDisk) {
                        rs = conn.createStatement().executeQuery("SELECT * FROM artist WHERE name = '" + s + "' AND server = 3");
                        if (!rs.next()) {
                            notFounded.add(s);
                        }
                    }
    ...
    private void updateDB() throws Exception {
        for (String artist : notFounded) {
            ...
            if (!conn.createStatement().executeQuery("SELECT * FROM artist WHERE name = '" + artist + "' AND server = 3").next()) {
