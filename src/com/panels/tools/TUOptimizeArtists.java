...
public class TUOptimizeArtists {
    ...
    public TUOptimizeArtists() {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    File check = new File(xml.getContentById("TUoutput"));
                    if (!check.exists()) {
                        check.mkdirs();
                    }
                    ...
                    ResultSet rs = conn.createStatement().executeQuery("SELECT name FROM artist WHERE server = 1");
                    while (rs.next()) {
                    ...
                    for (String s : onDisk) {
                        rs = conn.createStatement().executeQuery("SELECT * FROM artist WHERE name = '" + s + "' AND server = 1");
                        if (!rs.next()) {
                            notFounded.add(s);
                        }
                    }
                    ...
                    Logger.getLogger(TUOptimizeArtists.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }.start();
    ...
    private void updateDB() throws Exception {
        ...
        for (String artist : notFounded) {
            HtmlPage page = webClient.getPage("http://" + artist.toLowerCase() + ".tumblr.com/archive");
            Document parsed = Jsoup.parse(page.asXml());

            if (!page.asXml().contains("Whatever you were looking for doesn't currently exist at this address.")) {
                PreparedStatement statement;
                if (!conn.createStatement().executeQuery("SELECT * FROM artist WHERE name = '" + artist + "' AND server = 1").next()) {
                    ...
                    String avatar = parsed.select("img.avatar").get(0).attr("src");
                    avatar = avatar.substring(0, avatar.lastIndexOf(".") - 2) + "128.png";
