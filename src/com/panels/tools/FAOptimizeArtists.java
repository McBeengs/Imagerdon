...
public class FAOptimizeArtists {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    ResultSet rs = conn.createStatement().executeQuery("SELECT name FROM artist WHERE server = 2");
                    while (rs.next()) {
                    ...
                    for (String s : onDisk) {
                        rs = conn.createStatement().executeQuery("SELECT * FROM artist WHERE name = '" + s + "' AND server = 2");
                        if (!rs.next()) {
                            notFounded.add(s);
                        }
                    }
    ...
    private void updateDB() throws Exception {
        ...
        for (String artist : notFounded) {
            ...
            if (!page.asXml().contains("If this is your account and you would like to re-enable it,")
                    || !page.asXml().contains("This user cannot be found.")) {

                PreparedStatement statement;
                if (!conn.createStatement().executeQuery("SELECT * FROM artist WHERE name = '" + artist + "' AND server = 2").next()) {
