...
public class E621OptimizeArtists extends javax.swing.JFrame {
    ...
    private void initSubComponents() {
        setTitle("Optimize artists - e621");
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    File artistsXml = new File(xml.getContentById("E621output") + System.getProperty("file.separator") + "artists-log.xml");
                    ...
                    File check = new File(xml.getContentById("E621output"));
                    if (!check.exists()) {
                        check.mkdirs();
                    }
                    ...
                    for (int i = 0; i < temp.length - 1; i++) {
                        if (!temp[i].getAbsolutePath().equals(xml.getContentById("E621output")
                    ...
                    Logger.getLogger(E621OptimizeArtists.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }.start();
    }
    ...
    public void updateDB() {
        ...
        try {
            ...
            for (String artist : missing) {
                HtmlPage page = webClient.getPage("https://e621.net/post/index/1/" + artist.toLowerCase());

                if (!page.asXml().contains("No posts matched your search.")) {
                    String newTag = "<artist>\n<name>" + artist + "</name>\n" + "<avatarUrl>null</avatarUrl>\n"
                            + "<firstDownloaded>" + UsefulMethods.getSimpleDateFormat() + "</firstDownloaded>\n"
                            + "<lastUpdated>" + UsefulMethods.getSimpleDateFormat() + "</lastUpdated>\n<imageCount>";
            ...
            File file = new File(xml.getContentById("E621output") + System.getProperty("file.separator") + "artists-log.xml");
            if (file.exists()) {
                file.delete();
            } else {
                file.createNewFile();
